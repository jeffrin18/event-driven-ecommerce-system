...
The Product Service receives the message and reduces the item's stock in the database.
The Notification Service receives the same message and "sends an email" to the customer.
...
Simple Diagram: [Order Service] -> [RabbitMQ Exchange] -> (copy 1) -> [Product Service] -> [PostgreSQL DB]   -> (copy 2) -> [Notification Service]
...
Run the single command to build and launch the entire system:
docker compose up --build
Wait for all services to start. You will see logs indicating the database is initialized and the listeners are connected.
🧪 How to Test
